...
resourcename="banking"  # lower case, only
...
az group create --name banking_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group banking_rg --sku S1 --is-linux
az container create --resource-group banking_rg --name banking --image apilogicserver/banking:latest --dns-name-label banking --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//banking.westus.azurecontainer.io:5656'
